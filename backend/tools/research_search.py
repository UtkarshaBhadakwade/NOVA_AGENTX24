...
def research_search(query: str) -> List[Dict[str, Any]]:
    """
    ...
    try:
        encoded_query = urllib.parse.quote(f'all:"{clean_q}"' if " " in clean_q else f'all:{clean_q}')
        url = f"http://export.arxiv.org/api/query?search_query={encoded_query}&start=0&max_results=5"
        
        req = urllib.request.Request(
            url,
            headers={'User-Agent': 'AgentX-CompetitiveIntelligence/1.0'}
        )
        ...
        # If specific query returned no entries, fallback to broad search
        if not entries:
            fallback_url = "http://export.arxiv.org/api/query?search_query=all:%22AI%20agents%22&start=0&max_results=5"
            fallback_req = urllib.request.Request(fallback_url, headers={'User-Agent': 'AgentX-CompetitiveIntelligence/1.0'})
